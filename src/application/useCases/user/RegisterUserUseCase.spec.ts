...
describe("RegisterUserUseCase tests", () => {
  ...
  test("should register a user", async () => {
    ...
    expect(await repository.findById(savedUser.id)).toEqual({
      ...input,
  ...
  describe("Should throw an error: ", () => {
    test("when registering a user with email already registred in database", async () => {
      ...
      expect.assertions(2)
      await expect(sut.execute(input)).rejects.toEqual(
        new BusinessRuleError(ErrorMessage.ALREADY_REGISTRED("Email"))
      )
