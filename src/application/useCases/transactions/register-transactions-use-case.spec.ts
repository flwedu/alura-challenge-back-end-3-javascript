...
describe("Register Transactions use case tests", () => {
  async function setupRepositories() {
    ...
    const sut = new RegisterTransactionsUseCase(
      repositories.transactions,
      repositories.transactionsImports
    )

    const importId = await repositories.transactionsImports.save({
      id: "1",
      date: new Date(),
      transactionsDate: new Date(2022, 0, 1),
      userId: "1",
    })
  ...
  function generateTransaction(args?: [string, any][]) {
    ...
    args?.forEach((arg) => {
      const [key, value] = arg
      if (
        key in transaction.props &&
        typeof value == typeof transaction.props[key]
      ) {
  ...
  test("should register only one transaction if the dates are different", async () => {
    const { repositories, spy, sut, importId } = await setupRepositories()

    const input = [
      generateTransaction([["date", new Date(2022, 0, 1)]]),
      generateTransaction([["date", new Date(2022, 0, 2)]]),
    ]
