...
// Static routes
router.get("/", new VerifyCredentialsController(userRepository, encryptor).handle, (req, res) => new IndexViewController(transactionRepository).handle(req, res));
...
// POST Routes
router.post("/", upload.single("files"), (req, res) => new FileInputController(transactionRepository).handle(req, res));
