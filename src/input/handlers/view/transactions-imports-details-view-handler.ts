...
export const transactionsImportsDetailsViewHandler = (repositories: RepositoriesSource) =>

    async (request: Request, response: Response) => {

        const id = request.params.id;
        try {
            const transactionsImport = await new LoadTransactionImportByIdUseCase(repositories.transactionsImports).execute(id);
            const transactions = await new LoadAllTransactionsByImportIdUseCase(repositories.transactions).execute(id)
            const user = await new LoadUserByIdUseCase(repositories.users).execute(transactionsImport.userId);
